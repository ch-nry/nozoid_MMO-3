...
inline int32_t clip_S32_30(int32_t in1) {
  uint32_t out;
  asm volatile("ssat %0, 30, %1" : "=r" (out) : "r" (in1));
  return(out);
  //return(max(min(in1,0x3FFFFFFF), -0x3FFFFFFF));
  
}

inline int32_t clip_S32_29(int32_t in1) {
  uint32_t out;
  asm volatile("ssat %0, 29, %1" : "=r" (out) : "r" (in1));
  return(out);
}

inline int32_t clip_U32_31(uint32_t in1) { // Why is it lot's faster in C than in ASM???
  uint32_t out;
  asm volatile("usat %0, 31, %1" : "=r" (out) : "r" (in1));
  return(out);
}

inline int32_t clip_U32_30(uint32_t in1) {
  uint32_t out;
  asm volatile("usat %0, 30, %1" : "=r" (out) : "r" (in1));
  return(out);
}

inline int32_t clip_U32_29(uint32_t in1) {
  uint32_t out;
  asm volatile("usat %0, 29, %1" : "=r" (out) : "r" (in1));
  return(out);
}

inline int32_t clip_U32_28(uint32_t in1) {
  uint32_t out;
  asm volatile("usat %0, 28, %1" : "=r" (out) : "r" (in1));
  return(out);
}
