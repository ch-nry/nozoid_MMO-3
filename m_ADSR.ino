...
inline void ADSR_update() {
  ...
  //tmp = (table_CV2freq[0x2B0-(tmp>>7)]); // change the relation for faster adsr
  tmp = (table_CV2freq[0x330-(tmp/90)]); 
...
inline uint32_t ADSR() { 
  ...
  modulation_data_AM[mod_ADSR] = ((ADSR_out_tmp >> 14));
